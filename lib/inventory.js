...
export async function decrementInventory(productKey, amount = 1) {
  ...
  const next = Math.max(0, product.inventory_count - qty);
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from("gg_products")
...
export async function incrementInventory(productKey, amount = 1) {
  const product = await getProduct(productKey);
  if (!product || product.inventory_count == null) return product;

  const qty = normalizeAmount(amount);
  const next = product.inventory_count + qty;
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from("gg_products")
    .update({ inventory_count: next, updated_at: new Date().toISOString() })
